...
<body>
	...
	<c:if test="${articles.size() > 0}">
		...
		<table border="1">
			...
			<%-- 게시글 개수가 0개일 경우 목록 대신 "게시물이 존재하지 않습니다." 출력 --%>
			...
			<c:forEach var="article" items="${articles}" varStatus="num">
				<tr>
					...
					<td><a
						href="/database/board/content?boardNo=${article.boardNo}">${article.title}</a></td>
					<td><a
						href="/database/board/delete?boardNo=${article.boardNo}">[삭제]</a>
					</td>
				</tr>
			</c:forEach>
